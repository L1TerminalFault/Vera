...
int main() {
    ...
    // openOpts.defaultPath = std::filesystem::current_path().string().c_str();
    // // Default to runtime execution folder

    // std::filesystem::path chosenOpenPath =
    // shell.openFileDialog(openOpts).c_str(); if (!chosenOpenPath.empty()) {
    ...
    // saveOpts.defaultPath = std::filesystem::current_path().string().c_str();
    // saveOpts.defaultName = "untitled_export.json";
    //
    // std::filesystem::path chosenSavePath =
    // shell.saveFileDialog(saveOpts).c_str(); if (!chosenSavePath.empty()) {
    ...
    for (int i = 0; i < windowCount; ++i) {
        ...
        window->setCharCallback(
            [windowHandle, &app, &shell](std::string char_) {
                std::cout << "[Instance " << windowHandle << "] ";
                std::cout << "Key Pressed Character: `" << char_ + "`\n";
            });
        ...
        window->setKeyCallback([windowHandle, &app, &shell](
            ...
            // if (win) {
            //     bool isKPressed =
            //         (key == VeraKey::K) ? pressed :
            //         win->isPressed(VeraKey::K);
            //
            //     if (isKPressed) {
            //         std::cout << " | K Pressed";
